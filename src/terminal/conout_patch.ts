// node-pty Windows ConPTY 渲染进程补丁
// 内容与 patches/windowsConoutConnection.js 保持同步（由 tests/terminal_conout_patch.test.ts 校验）
// 说明：上游 node-pty 在 Worker 线程中排空 ConPTY 输出管道以避免
// ClosePseudoConsole 死锁（microsoft/node-pty#375），但 Obsidian 的
// Electron 渲染进程无法使用 worker_threads，因此改为主线程内联 socket 管道。

export const WINDOWSCONOUT_PATCH = String.raw`"use strict";
/**
