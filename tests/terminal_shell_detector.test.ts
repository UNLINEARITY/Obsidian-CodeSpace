...
describe("resolveShell", () => {
	...
	describe("macOS", () => {
		...
		it("falls back to /bin/zsh", async () => {
			const ctx = makeContext(MAC, {}, ["/bin/zsh"]);
			await expect(resolveShell(ctx)).resolves.toMatchObject({ file: "/bin/zsh", displayName: "zsh" });
		});
	});
	...
	describe("linux", () => {
		it("uses $SHELL when it exists", async () => {
			...
			await expect(resolveShell(ctx)).resolves.toMatchObject({ file: "/usr/bin/zsh", displayName: "zsh" });
		});

		it("falls back to /bin/bash then /bin/sh", async () => {
			const ctx = makeContext(LINUX, { SHELL: "/nonexistent/fish" }, ["/bin/sh"]);
			await expect(resolveShell(ctx)).resolves.toMatchObject({ file: "/bin/sh", displayName: "sh" });
		});
	});
	...
	describe("override", () => {
		...
		it("treats whitespace-only override as empty", async () => {
			const ctx = makeContext(MAC, {}, ["/bin/zsh"], "   ");
			await expect(resolveShell(ctx)).resolves.toMatchObject({ file: "/bin/zsh" });
		});
	});
